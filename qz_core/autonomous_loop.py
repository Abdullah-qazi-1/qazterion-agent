...
class AutonomousRunner:
    ...
    def run_task(
        ...
        try:
            with task_context_scope(task_ctx):
            ...
            t_info = get_task(task_id)
            task_status_str = str(t_info.get("status", "COMPLETED")).upper() if t_info else "COMPLETED"

            if task_status_str in ("FAILED", "CANCELLED", "BLOCKED"):
                final_status = "failed" if task_status_str != "CANCELLED" else "cancelled"
                self.storage.complete_task(task_id, status=final_status, latency_ms=latency_ms)
                self.event_bus.emit("TASK_FAILED" if final_status == "failed" else "TASK_CANCELLED", task_id, {
            ...
            if self.on_permission_request:
                configure_security_gateway(ask_handler=None)
